...
#pid = PID(target=0.20, kp=1.2, ki=0.00001, kd=0.4)
pid = PID(target=0.20, kp=0.3, ki=0.0001, kd=0.000001)
...
def clbk_laser(msg):
    ...
    regions_ = {
        ...
        'front2':  find_nearest (msg.ranges[330:360]),
...
def movement():
    ...
    print("Min distance in front region: ", regions['front1'],regions['front2'])
    
    #create an object of twist class, used to express the linear and angular velocity of the turtlebot 
    msg = Twist()
    
    pid_result = pid(regions['right'])
    print(f"Min distance in right region: {regions['right']}")
